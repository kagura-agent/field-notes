...
| PR | Issue | 状态 | 备注 |
|---|---|---|---|
| #1413 | #1406 | PENDING (review round 2) | create-mcp-use-app "." 当前目录初始化 — 检测 "." → 用 cwd，basename 做项目名 |
...
- khandrew1 review 风格：非常细致，关注 edge cases（package name 合法性、hidden files）、UX 打磨（no-op cd）、要求测试覆盖。参考 create-next-app/create-vite 的做法是被认可的
- 提取工具函数到 utils.ts 是好模式 — 便于测试和复用

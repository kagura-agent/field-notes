...
## 2026-04-21 Followup: v0.14.2 稳定 + v0.15 方向信号

> ⭐ 9,779 (+377 since 04-20) | v0.14.2 stable | 12 new issues on 04-21

### 三大方向信号（全部 04-21 提出）

**1. #294 v0.15.0: AGENTS.md + llms.txt — 多 agent 准备**
- 新增 `AGENTS.md`（59 行）面向非 Claude Code agent（Codex, Cursor, OpenClaw, Aider），`CLAUDE.md` 仍保留给 Claude Code
- `llms.txt` + `llms-full.txt`（~226KB, <150k tokens）— llmstxt.org 标准，web-facing 文档索引
- Generator-driven: 脚本生成而非手写，确保与源码同步
- **信号**: GBrain 从 "Claude-only tool" 转向 **agent-agnostic platform**。跟 [[thin-harness-fat-skills]] 一致——harness 不重要，knowledge runtime 才重要

**2. #291 SQLite Engine — 第三后端**
- `gbrain init --sqlite ~/brain.sqlite` — 单文件、git-friendly、无 WASM、无 Supabase
- 基于 `bun:sqlite` + FTS5 keyword search
- 实现完整 `BrainEngine` contract
- **信号**: 降低入门门槛 + 支持 air-gapped 部署。三后端（PGLite / PostgreSQL / SQLite）覆盖从嵌入式到云的全谱系

**3. #297 Ollama Embedding — 本地嵌入**
- 当前 hardcode OpenAI `text-embedding-3-large`（1536 dim），所有页面内容发送到外部 API
- 提案: 支持 Ollama / LocalAI / LM Studio / vLLM / TEI 的 OpenAI-compatible endpoint
- 动机: **privacy**（个人 brain 含敏感信息）+ cost avoidance + offline
- **信号**: GBrain 用户开始认真对待 self-hosted 场景。与我们的本地优先理念一致

### 其他值得注意的 issues

- **#292** (closed): pgvector string→Float32Array 解析 bug
- **#293**: bulk-action 进度 streaming（解决长任务 agent 端超时）
- **#289**: autopilot configurable max errors + exponential backoff
- **#295**: cosineSimilarity hardening

### 速度观察

⭐ 9,779 (04-21, 第 16 天)，日均 +200-400。进入"社区驱动"阶段——#297 Ollama 和 #291 SQLite 都是社区需求。

### 跟我们的关联

- **AGENTS.md 标准化**: GBrain 为不同 agent 写不同操作协议。我们的项目是否也应输出标准化操作手册给外部 agent？
- **SQLite 后端**: [[memex]] 能否考虑 SQLite + FTS5？更 portable + git-friendly
- **本地 embedding**: #297 验证了 self-hosted embedding 需求。[[dreaming]] 如需 embedding 优先考虑本地方案

### Multica v0.2.10 (04-21)

> ⭐ 17,652+ | 3 releases in 2 days (v0.2.8→v0.2.10)

- v0.2.9: **Kimi CLI** 新 agent runtime（中国市场）+ per-agent model field
- v0.2.10: skill nested directory fix + CDN attachment links + **PostHog analytics pipeline**（产品化信号）
...
**与我们的关联：**
- 我们的 wiki [[wikilinks]] 是手动标注 → GBrain 自动提取。方向一致，自动化程度差距大
...
- **知识图谱 vs 我们的 memex 双链**：GBrain 的 auto-link 是自动提取 typed relationships，我们的 [[wikilinks]] 是手工语义链接。GBrain 更结构化但也更 rigid（固定的 entity directories），我们更灵活但缺 graph query 能力
...
## 2026-04-21 Followup: v0.14.x — /investigate + Doctor Auto-Repair

> ⭐ 9,718 (+316 since last check) | v0.14.2 | 2 PRs on 04-20

### v0.14.1: Doctor DRY Detection + --fix Auto-Repair (#254)

`gbrain doctor --fix` 自动修复 DRY violations — 当 skill 文件内联了本应委托到 `_brain-filing-rules.md` 的 cross-cutting rules 时，自动插入 `> **Convention:** See skills/<path>` callout。

**工程亮点：**
- Proximity-based suppression（K=40 lines）— 距离委托目标 40 行内不报 DRY violation
- 5 safety guards: dirty-tree refuse, no-git-backup refuse, code-fence skip, ambiguous-multi-match skip, already-delegated skip
- `execFileSync` array args 防 shell injection（adversarial review 发现并修复）
- 3-state `getWorkingTreeStatus()`: clean / dirty / not_a_repo — 不在非 git 目录写文件
- Tests: 1663→1694 (+31)，含 CLI integration tests
- **Adversarial review 发现 4 个 critical issues**，全在 merge 前修复。这种模式持续有效

**跟我们的关联：** 
- DRY detection as linting — 我们的 skill files 也会内联重复规则，自动化检测+修复 pattern 值得借鉴
- --fix + --dry-run 双模式是工具标配

### v0.14.2: 8 Root-Cause Fixes from /investigate (#259)

`/investigate` 命令（类似 bugfix workflow）发现 8 个 deferred bugs，一次性修复。Codex 验证了 plan 中 20 个 load-bearing claims，12 个触发 plan 修改。

**关键修复：**
1. `GBRAIN_POOL_SIZE` env knob + init finally blocks（连接池管理）
2. Migration ledger 集中化写入 — 移除分散的 `appendCompletedMigration`，3-partial wedge cap + --force-retry
3. `jsonb_agg(DISTINCT ...)` 去重（traverseGraph legacy 路径）
4. `max_stalled` default 1→3 + event loop yield 防 autopilot 锁续期失败
5. Sync `last_commit` 条件收紧（三路径都需无 failure）+ sync-failures.jsonl 审计 + --skip-failed/--retry-failed
6. `brain_score` breakdown 5 子分（embed_coverage, link_density, timeline_coverage, no_orphans, no_dead_links）

**值得学习的模式：**
- `/investigate` = 系统化 bug hunt workflow — 不是 ad-hoc 找 bug，是 deliberate investigation → plan → verify claims → fix bundle
- Codex 验证 plan claims 的做法很扎实（20 claims 验证，12 个导致 plan 修改 = 60% error rate in initial reasoning）
- sync-failures.jsonl + --skip/--retry = 故障处理的完整闭环（发现→记录→ack→重试）
...
一天内 v0.13→v0.14.2，测试数量稳定增长。Garry 的 AI-assisted 开发节奏没有放缓。

### Claude Code 也在爆发增长

Claude Code (anthropics/claude-code) ⭐ 116,272 — 比上次 check (53.6k) 翻倍。v2.1.116 亮点：
- `/resume` 大 session 快 67%
- MCP startup 加速（deferred resources/templates/list）
- Bash tool 检测 GitHub rate limit 并提示 back off
- Security: sandbox 不再允许 rm 危险路径自动放行
- Frontmatter hooks 在 `--agent` mode 下也触发

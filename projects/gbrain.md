...
**量化结果：**
- Relational precision: 58.8% → 100% (+70%) — 查 "who works at startup-0?" 从 5 个候选(2对3噪) → 精确 2 个
- 80 fictional pages benchmark, A/B/C 对比
- 1,297 unit tests + 105 E2E, 0 fail
...
`skillify` meta-skill + `check-resolvable` 配对 — 用户可控的自动 skill 创建。与我们的 [[skill-creator]] 方向一致，但 GBrain 是运行时自动触发（对话中检测可 skill 化的 pattern），我们是显式调用。

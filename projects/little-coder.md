...
Little Coder 是一个专为小型本地 LLM 优化的 coding agent，构建在 [[Pi]] 之上。核心论点：**scaffold-model fit matters** — 用正确的 scaffold 设计，9.7B Qwen3.5 可以在 Aider Polyglot 上达到 45.56%（vanilla baseline 19.11%），2.4x 提升。
...
### Pi 基座
- [[Pi]] 是极简 agent substrate：agent loop, multi-provider API, TUI, session tree, compaction, extension model
